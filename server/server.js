...
const badgeFlags = [
  ...
  { bit: 1 << 3, label: "Bug Hunter Lv1" },
  { bit: 1 << 6, label: "House Bravery" },
  { bit: 1 << 7, label: "House Brilliance" },
  { bit: 1 << 8, label: "House Balance" },
  { bit: 1 << 9, label: "Early Supporter" },
  { bit: 1 << 14, label: "Bug Hunter Lv2" },
  { bit: 1 << 16, label: "Verified Bot" },
  { bit: 1 << 17, label: "Early Verified Bot Dev" },
  { bit: 1 << 19, label: "HTTP Interactions" }
];
...
app.listen(port, () => {
  console.log(`Discord profile showcase running on http://localhost:${port}`);
});
